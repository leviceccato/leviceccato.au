...
export const routes = Object
	...
	.flatMap<Route>(([path, meta]) => {
		...
		segments = segments.flatMap((segment, segmentIndex) => {
			 ...
			 * Ignore indexes
			 */
			if (segmentName === 'index' || !segmentName) {
				return []
			}
...
export function getNextRoute(
	...
	let route = routes[routeIndex + currentDirection]
	while (route?.isHidden) {
		currentDirection += direction
		route = routes[routeIndex + currentDirection]
	}
